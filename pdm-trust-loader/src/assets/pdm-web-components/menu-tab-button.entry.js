...
<ion-content>
  ...
  _getMenuMode() {
    ...
    return (h("ion-item", { lines: "none", class: `menu-btn ion-margin-start nav-menu-item${!!this.selected ? " tab-selected" : ''}`, button: true, onClick: (evt) => !hasOptions ? this.navigateToTab(tabName) : this.getPopOver(evt, this.tab) }, h("ion-grid", null, h("ion-row", { class: "ion-align-items-center ion-justify-content-center ion-padding-top" }, this._getIcon()), h("ion-row", { class: "ion-align-items-center ion-justify-content-center ion-padding-bottom" }, h("span", { class: "menu-text" }, h("ion-note", { class: "menu-text ion-text-center ion-text-wrap" }, this.label)))), this.getBadge()));
  }
  _getTabBarMode() {
    const hasOptions = typeof this.tab !== 'string';
    const tabName = !hasOptions ? this.tab : this.tab.label;
