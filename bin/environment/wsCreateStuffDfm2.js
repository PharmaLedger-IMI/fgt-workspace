/**
 * Fork of wsCreateStuff.js for a specific DFM situation:
 * https://github.com/PharmaLedger-IMI/fgt-workspace/issues/119#issuecomment-1283748884
 * 
 * Delete it, if you do not know what you would need it for... :-)
 * 
 */
...
const shipmentsCreateTestDfm = async function (conf, sender) {
    ...
    for (const product of sender.products) { // ship all DFM products
        const gtin = product.gtin;
	if (gtin=='02113100000028' || gtin=='07613421039424') continue;
...
(async () => {
    ...
    } else if (conf.scenario == ScenarioEnum.dfm2) {
	...
	for(const gtin of ['37482919347386']) { // INCMedin only
	    for (const batch of mahActor.batches[gtin]) {
                const batchNumber = batch.batchNumber;
                const quantity = batch.quantity;
                //const serialNumber = batch.serialNumbers[0];
		const fakeSaleRes = {
  shipmentId: 'MAH251339219-16661728368639677850',
  orderId: 'WHS000001-2022-10-19T09:47:16.820Z',
  requesterId: 'PHA000001',
  senderId: 'WHS000001',
  status: {
    status: 'received',
    log: [
  ...
  shipmentLines: [
    {
      ...
      requesterId: 'PHA000001',
      status: {},
      createdOn: 1666172836863
    }
  ]
};
                SHIPMENTS_ON_PHA.push(fakeSaleRes);
            }
	}
