...
const {MSD, PFIZER, ROCHE, BAYER, NOVO_NORDISK, GSK, TAKEDA, SANOFI, generatePharmacyCredentials, generateWholesalerCredentials} = require('./credentials/credentialsTests');
...
const getFakeTest = () => {
    const SINGLE = {}
    SINGLE[APPS.MAH] = [getCredentials(APPS.MAH, undefined, true)];
    SINGLE[APPS.WHOLESALER] = [getCredentials(APPS.WHOLESALER), getCredentials(APPS.WHOLESALER)];
    SINGLE[APPS.PHARMACY] = [getCredentials(APPS.PHARMACY), getCredentials(APPS.PHARMACY)];
    return SINGLE;
}

const getTest = () => {
    const MULTIPLE = {};
    MULTIPLE[APPS.MAH] = [PFIZER, MSD, ROCHE, BAYER, NOVO_NORDISK, GSK, TAKEDA, SANOFI];
