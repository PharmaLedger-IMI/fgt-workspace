...
let testName = 'StockManagerIntegrationTest' // no spaces please. its used as a folder name (change for the unit being tested)
...
const runTest = function(callback){
    ...
    getMockParticipantManager(domain, credentials, (err, participantManager) => {
        ...
        utils.generateStockWithManagers(participantManager, (err, stock)=>{
            if(err)
                return callback(err);
