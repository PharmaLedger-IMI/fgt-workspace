...
const calculateTraceability = function(participant, sale, callback){
    let traceabilityManager;

    try {
        traceabilityManager = participant.getManager("TraceabilityManager");
    } catch (e) {
        return callback(e);
    }
    ...
    setTimeout(() => {
        traceabilityManager.getOne(product, (err, startNode, endNode, nodeList) => {
            if (err)
                return callback(err);
            callback(undefined, startNode, endNode, nodeList);
        });
...
create(testOpts, (err, results) => {
    ...
    issueSale(results['fgt-pharmacy-wallet'][0].manager, results['fgt-mah-wallet'][0].results[1], (err, sale) => {
        if (err)
            return cb(err);
        calculateTraceability(results['fgt-mah-wallet'][0].manager, sale, (err, startNodeMah, endNodeMah, nodeListMah) => {
            if (err)
                return cb(err);
            calculateTraceability(results['fgt-pharmacy-wallet'][0].manager, sale, (err, startNodePha, endNodePha, nodeListPha) => {
                cb(err, results, startNodeMah, startNodePha, endNodeMah, endNodePha, nodeListMah, nodeListPha);
            });
        });
    });
});
